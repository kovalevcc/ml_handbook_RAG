...
class ThresholdRetriever(BaseRetriever):
    """
    Кастомный ретривер для поиска документов по косинусной близости с пороговым значением.

    Атрибуты:
        vectorstore (Any): Хранилище векторов для поиска.
        embeddings (Any): Модель для получения векторного представления запросов.
        threshold (float): Минимальное значение косинусной близости для отбора документов (по умолчанию 0.05).
        k (int): Количество возвращаемых топовых документов (по умолчанию 5).

    Методы:
        get_relevant_documents(query: str) -> List[Document]:
            Возвращает документы, соответствующие пороговому значению.
    """
